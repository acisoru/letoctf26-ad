...
vector<Chat> db_get_chats(const int &user_id) {
    ...
    p.reserve(1);
    p.append(user_id);
    auto r = W.exec_params("SELECT id, is_deleted FROM Chats WHERE user_id = $1 AND is_deleted = FALSE ORDER BY id", p);
    vector<Chat> res;
...
vector<Message> db_get_messages(const int &chat_id) {
    ...
    p.reserve(1);
    p.append(chat_id);
    auto r = W.exec_params("SELECT id, text, direction FROM Messages WHERE chat_id = $1 ORDER BY id", p);
    vector<Message> res;
    for (auto const &row: r){
        int id = row[0].as<int>();
